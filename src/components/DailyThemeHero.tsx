...
export default function DailyThemeHero({
  ...
  return (
    <div className="relative rounded-2xl bg-gradient-to-r from-[#292232] via-[#2F263A] to-[#251E2E] border border-[#382D4F] p-4 sm:p-7 overflow-hidden shadow-2xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 sm:gap-6 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 max-w-2xl">
          ...
          <div className="relative w-20 h-20 sm:w-28 sm:h-28 rounded-2xl overflow-hidden flex-shrink-0 bg-[#1D1728] border border-[#3E2F54] p-1.5 shadow-xl flex items-center justify-center">
            <img
          ...
          <div className="space-y-2.5 sm:space-y-3 min-w-0">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#1A161F]/80 border border-[#382D4F] text-[10px] sm:text-[11px] font-medium tracking-wide text-[#A89CBF] uppercase">
            ...
            <h1 className="text-xl sm:text-3xl lg:text-4xl font-bold text-[#F3EEFE] tracking-tight">
              {theme.tag}
            </h1>
            ...
            <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 pt-1">
              <button
                onClick={onPlayTheme}
                className="flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-[#7B61FF] hover:bg-[#8D76FF] text-white text-xs font-semibold shadow-lg shadow-[#7B61FF]/30 transition-all hover:scale-105 active:scale-95 cursor-pointer"
              >
                <Play className="w-3.5 h-3.5 fill-white shrink-0" />
                <span>{isPlayingTheme ? "Pause episode" : "Play featured episode"}</span>
              </button>

              <span className="text-[11px] sm:text-xs text-[#8F7FA8] font-mono whitespace-nowrap">
                🎙️ {theme.episode_count ?? theme.song_count ?? 0} {(theme.episode_count ?? theme.song_count) === 1 ? 'episode' : 'episodes'}
              </span>
            </div>
          </div>
        </div>
